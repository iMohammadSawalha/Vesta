...
import { useParams } from "react-router-dom";
  const InviteUserForm = ({
    ...
    const addUserToWorkspace= async()=>{
      try{
      const response = await axiosPrivate.post("/api/workspace/invite",{
        email,
        url
      },
      {
    ...
    return (
      ...
      />
        ...
        >
          <input value={email} onChange={(event)=>setEmail(event.target.value)} className="invite-forum-input" placeholder="Enter email ..."/>
          <button
